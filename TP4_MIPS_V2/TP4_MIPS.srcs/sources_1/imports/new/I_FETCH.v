...
module I_FETCH(input clk, PCSrc, rst, input wire [31:0] EX_MEM_NPC, 
	...
	PC pc(.clk(clk), .rst(rst), .npc(mux_npc_wire), .PC(pc_wire));
	
	INSTR_MEM mem(.addr(pc_wire), .data(data_wire));
	
	INCR incr(.pcin(pc_wire), .pcout(npc_wire));
	
	IF_ID if_id(.clk(clk), .rst(rst), .npc(npc_wire), .instr(data_wire), .instrout(IF_ID_IR), 
		.npcout(IF_ID_NPC));
endmodule
